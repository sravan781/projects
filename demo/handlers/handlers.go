...
func Deposite_money(c *gin.Context) {
	...
	var User model.Deposite
	var user1 model.Deposite
	...
	if user1.Pin == pin1 {
		...
		c.JSON(200, gin.H{"message": fmt.Sprintf("credited %d in account  \n  total balance %d", amount, amount+user1.Balance)})
		return
	}

	c.JSON(200, gin.H{"message": fmt.Sprintf("invalid pin or account number")})

}

func Transfer_money(c *gin.Context) {
	var User model.Transfer
	var user1 model.Deposite
	var user2 model.Deposite
...
func Withdrawal(c *gin.Context) {
	...
	if user1.Pin == pin1 {
		...
		fmt.Println("deposited", res)
		fmt.Println("deposited", res1)
		c.JSON(200, gin.H{"message": fmt.Sprintf("debited %d from account  \n   total balance %d", amount, user1.Balance-amount)})
		return
	}
	c.JSON(200, gin.H{"message": fmt.Sprintf("invalid pin or account number")})
}
...
func Resetting(c *gin.Context) {
	var temp model.Reset
	var user1 model.Reset
